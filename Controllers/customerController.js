...
module.exports = {
  getAllOrOne: async (req, res, next) => {
    ...
    if (!id) {
      try {
        const allCustomer = await Customers.find({}).populate(
      ...
      try {
        const customer = await Customers.findById(req.params.id).populate(
          [{ path: "customerAddresses" },
          { path: "cart" },
          { path: "myFavorite" }]
        );     
  ...
  addCustomer: (req, res, next) => {
    ...
    customer.save().catch((error) => next(error + "cannot add customer"));
